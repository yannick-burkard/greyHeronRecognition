...
def img2label_paths(img_paths):
    ...
    for img_path in img_paths:
        ...
        if label_path[-14]=='1':
            id=27
            label_path=label_path[:-id]+'1'+label_path[-id:]
            #print(label_path)
        label_paths.append(label_path)
    return label_paths
    #######Yannick: original code, changed to adapt to my environment
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p) as t:
                        ...
                        ######t = t.read().strip().splitlines(encoding='latin1')
            ...
            self.im_files = sorted(x.replace("/", os.sep) for x in f if x.split(".")[-1].lower() in IMG_FORMATS) #oversampled
        ...
        # Check cache
        self.label_files = img2label_paths(self.im_files)  # labels #oversampled
        ...
        try:
            ...
            ###cache, exists = self.cache_labels(cache_path, prefix), False  # run cache ops ###creates cache first time! make it save it such that it keep repeated, and replace lines with cache below in proper way!
             ######replaced Yannick (1 line)
            cache, exists = self.cache_labels_repeat(cache_path, prefix), False ###added Yannick
        ...
        ######replaced Yannick (1 line)
        im_paths, labels, shapes, self.segments = zip(*cache['ls_im_lb_sh_se'])
        ...
        ######replaced Yannick (2 lines)
        self.im_files = list(im_paths)  # update
        self.label_files = img2label_paths(im_paths)  # update
    ...
    ################(added Yannick)################
    def cache_labels_repeat(self, path=Path("./labels.cache"), prefix=""):
        # Cache dataset labels, check images and read shapes
        x = {}  # dict
        ls_im_lb_sh_se = []
        nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found, empty, corrupt, messages
        ...
        with Pool(NUM_THREADS) as pool:
            ...
            #counter=0
            #loop goes through all images (even repeated)
            for im_file, lb, shape, segments, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                #counter+=1
                #print(counter)
                nm += nm_f
                ...
                if im_file:
                    #x[im_file] = [lb, shape, segments]
                    ls_im_lb_sh_se.append((im_file,lb,shape,segments))
                if msg:
                    msgs.append(msg)
                pbar.desc = f"{desc} {nf} images, {nm + ne} backgrounds, {nc} corrupt"
        ...
        if nf == 0:
            LOGGER.warning(f"{prefix}WARNING ⚠️ No labels found in {path}. {HELP_URL}")
        x["ls_im_lb_sh_se"] = ls_im_lb_sh_se
        x["hash"] = get_hash(self.label_files + self.im_files)
        x["results"] = nf, nm, ne, nc, len(self.im_files)
        x["msgs"] = msgs  # warnings
        x["version"] = self.cache_version  # cache version
